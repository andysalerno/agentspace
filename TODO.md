...
- [x] CLEANUP: Avoid blocking filesystem work under async locks in `SkillRegistry`. The service currently wraps synchronous `std::fs` operations in `tokio::RwLock` and calls them from async handlers; consider `spawn_blocking`, `tokio::fs`, or a clearer synchronous service boundary.
- [x] CLEANUP: Preserve underlying error sources instead of converting them into strings in `AgentHostError` and `GatewayError`. Prefer variants that hold source errors directly, or use `thiserror`, so callers and logs retain structured error chains.
- [x] CLEANUP: Reduce stringly typed kernel event handling. `KernelEvent.event_type` is a raw `String`, and status derivation checks the literal `"session/status"`; add known event constants/helpers or an enum with an `Unknown(String)` escape hatch.
