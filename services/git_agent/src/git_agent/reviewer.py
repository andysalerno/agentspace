...
def build_review_prompt(context: ReviewContext) -> str:
    ...
    return (
        ...
        "when commenting on a binary patch. Review in prototyping mode: patches "
        "to main may be non-production-ready, so do not reject solely for "
        "missing comprehensive tests, incomplete hardening, rough architecture, "
        "TODOs, or polish issues. Prefer acceptance when the patch is coherent "
        "and has a reasonable path to run or inspect. Reject only for concrete "
        "blockers such as broken claimed functionality, syntax/runtime failures, "
        "unsafe or secret-bearing changes, data-loss risks, invalid patches, or "
        "changes that cannot be reasonably validated. Each independent project "
        "should provide a justfile recipe named validate, but a minimal smoke, "
        "static, build, or placeholder sanity check is acceptable when no tests "
        "exist yet. GitAgent is the final authority; no human override. "
